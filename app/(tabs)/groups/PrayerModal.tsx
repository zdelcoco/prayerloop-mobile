...
import { addGroupPrayer } from '@/store/groupPrayersSlice';
...
export default function PrayerModal({
  ...
  const route = useRoute<{
    key: string;
    name: string;
    params: { mode: 'add' | 'edit'; groupProfileId: number; prayer?: Prayer };
  }>();
  ...
  const handleAddPrayer = useCallback(async () => {
    ...
    try {
      await dispatch(addGroupPrayer(route.params.groupProfileId, prayerData));
      resetForm();
